...
CLOUDFLARE_ACCESS_ENABLED = _env_bool("CLOUDFLARE_ACCESS_ENABLED", default=False)
CLOUDFLARE_ACCESS_TEAM_DOMAIN = (
    os.getenv("CLOUDFLARE_ACCESS_TEAM_DOMAIN") or ""
).strip()
CLOUDFLARE_ACCESS_AUD = (os.getenv("CLOUDFLARE_ACCESS_AUD") or "").strip()
CLOUDFLARE_ACCESS_JWKS_CACHE_SECONDS = max(
    60,
    _env_int("CLOUDFLARE_ACCESS_JWKS_CACHE_SECONDS", 3600),
)
CLOUDFLARE_ACCESS_HTTP_TIMEOUT_SECONDS = max(
    1,
    _env_int("CLOUDFLARE_ACCESS_HTTP_TIMEOUT_SECONDS", 5),
)
CLOUDFLARE_ACCESS_MAX_TOKEN_AGE_SECONDS = max(
    60,
    _env_int("CLOUDFLARE_ACCESS_MAX_TOKEN_AGE_SECONDS", 8 * 60 * 60),
)
...
SESSION_MAX_AGE_SECONDS = _env_int("SESSION_MAX_AGE_SECONDS", 8 * 60 * 60)
...
# Solo se reintenta automaticamente si no se ha establecido la conexion TLS.
...
N8N_CA_CERT_FILE = (os.getenv("N8N_CA_CERT_FILE") or "").strip()
